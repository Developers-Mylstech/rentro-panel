...
export default function AddProduct() {
  ...
  // In your component state, replace showSellForm and showRentForm with:
  const [visibleForm, setVisibleForm] = useState(null); // 'sell', 'rent', or null

  // Update the onOptionChange function
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-start w-full py-2 gap-2 ">
        ...
        {/* flex flex-wrap md:flex-nowrap md:justify-between gap-4 md:w-[70%] w-full relative */}
        ...
        <div className="grid md:grid-cols-4 grid-cols-1 gap-4 md:w-[70%] w-full relative">
          <div className="flex   flex-col gap-2 pr-4 border-r-2 border-gray-200 dark:border-gray-600 relative">
            <div className="flex justify-start items-center gap-2">
              ...
              />
              <label htmlFor="Sell" className="text-green-600  text-sm flex justify-between w-full ">
                <span>Sell</span> { sellFormData?.discountedPrice && <span className="font-bold text-gray-500 bg-green-100 p-1 rounded-md">{  sellFormData?.discountedPrice + " AED"}</span> } 
              </label>
            </div>
          ...
          <div className="flex flex-col gap-2 pr-4 border-r-2 border-gray-200 dark:border-gray-600 relative">
            <div className="flex justify-start items-center gap-4">
              <Checkbox
                ...
                className="border border-gray-300 dark:border-gray-600 rounded-md h-5 w-5"
              />
              <label htmlFor="Rent" className="text-orange-500   text-sm flex justify-between w-full ">
                Rent { rentFormData?.discountedPrice && <span className="font-bold text-gray-500 bg-orange-100 p-1 rounded-md">  {rentFormData?.discountedPrice + " AED"}</span> } 
              </label>
            </div>
...
<div className="flex flex-col gap-4 ">
      ...
      <div className="flex md:flex-row flex-col justify-between items-center w-full">
        <label className="font-semibold text-gray-500 text-left w-full md:w-auto my-2">Specifications</label>
      ...
      {customFields.length > 0 && (
        <div className="flex flex-col gap-3  w-full my-2">
          {customFields.map((field, index) => (
            <div key={field.code} className="flex justify-between items-center ">
              <label className=" font-medium text-gray-500">{field.label}</label>
              <input
                type="text"
                value={field.value}
                onChange={(e) => handleCustomFieldChange(field.code, e.target.value)}
